...
//    public double report(int patientID) throws SQLException{
//        double result = 0;
//        String query = "select (sum(i.dailyrate)+ sum(i.supplies) + sum(i.services)+ sum(m.unitcost * m.units) +\n" +
//        "sum(s.roomfee) + sum(s.surgeonfee) + sum(s.SUPPLIES)) \n" + "\"Total cost\"\n" +
//        "from surgical s, medication m, inpatient i\n" + "where s.patientid = m.patientid AND\n" + 
//        "m.patientid = i.PATIENTID AND\n" + "s.patientid = ?";
//        
//        try(Connection connection = DriverManager.getConnection(url, user, password);
//                PreparedStatement ps = connection.prepareStatement(query)){
//            ps.setInt(1, patientID);
//            try(ResultSet rs = ps.executeQuery()){
//                while(rs.next()){
//                    result = rs.getDouble("Total cost");
//                }
//            }
//        }
//        return result;
